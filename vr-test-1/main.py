...
class MyApp(ShowBase):
    def __init__(self):
        ...
        self.buffer_left = self.win.makeTextureBuffer("buffer_left", 2048, 2048)
        self.buffer_right = self.win.makeTextureBuffer("buffer_right", 2048, 2048)
...
class main:
    def start(self):
        ...
        with xr.ContextObject(
            ...
            for frame_index, frame_state in enumerate(context.frame_loop()):
                ...
                frame_left = self.capture_frame(app.buffer_left)
                frame_right = self.capture_frame(app.buffer_right)
                ...
                # try:
                # Ensure OpenGL context is ready for texture update
                GL.glUseProgram(shader_program)
    ...
    def capture_frame(self, buffer):
        tex = buffer.getTexture()
        tex_data = memoryview(tex.getUncompressedRamImage()).tolist()
        img = np.array(tex_data, dtype=np.uint8).reshape((tex.getYSize(), tex.getXSize(), tex.getNumComponents()))
        return img

    def update_image_offset(self, new_offset):
        self.image_offset += new_offset
        print(f"Image offset updated to: {self.image_offset}")
